/**
 * Draggable, looping horizontal card carousel.
 * Bootstraps automatically on any [data-drag-carousel] viewport
 * containing a [data-drag-track] child.
 */
(function () {
    function initDragCarousel(viewport, track, cardSelector) {
        ...
        function getOriginalCards() {
            return Array.from(track.querySelectorAll(cardSelector + ':not([data-loop-clone])'));
        }
        ...
        function resolveCardFromTarget(target) {
            ...
            return target.closest(cardSelector + '[data-href]');
        }
        ...
        function bindCardLinks(scope) {
            ...
            scope.querySelectorAll(cardSelector + '[data-href]').forEach(function (card) {
    ...
    document.querySelectorAll('[data-drag-carousel]').forEach(function (viewport) {
        var track = viewport.querySelector('[data-drag-track]');
        if (!track) {
            return;
        }
        initDragCarousel(viewport, track, viewport.getAttribute('data-drag-carousel') || '.playground-card');
    });
})();
